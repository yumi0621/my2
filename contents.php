...
class contents{
    ...
    //하나씩 불러온 데이터를 array_push를 이용하여 배열 $content에 데이터를 하나씩 추가
        while ($row = $res->fetch_array(MYSQLI_ASSOC)) {
            ...
            //댓글을 반환하는 메소드를 호출함, 아규먼트로게시물번호전달
            $commentLoad = $this->commentLoad($row['contentsID']);

            //게시물의 댓글을담는 배열을 생성, 댓글은 여러 개일 수 있으므로 배열로 생성
            $row['comment'] = array();
...
//LIMIT이 있는 이유는 최초 등록된 20개의 게시물만 출력하기 위함
$sql = "SELECT c.contentsID, c.myMemberID, c.content, c.regTime, m.userName, m.profilePhoto FROM contents c JOIN mymember m ON (c.myMemberID = m.myMemberID) {$sqlMaker} ORDER BY c.regTime DESC LIMIT {$limitFirstValue}, {$dataCount}";
$this->dbConnection();
$res = $this->dbConnection->query($sql);
...
//하나씩 불러온 데이터를 array_push를 이용하여 $content에 데이터를 하나씩 추가
        while ($row = $res->fetch_array(MYSQLI_ASSOC)) {
            ...
            //게시물의 댓글을 담는 배열 생성, 댓글은 여러개 일수 있으므로 배열로 생성
            $row['comment'] = array();
